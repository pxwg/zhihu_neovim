...
---@param html_content html_content HTML content to be converted
---@return md_content md_content Converted Markdown content or an error message
---@return string|nil error
function M.convert_html_to_md(html_content)
  local plugin_root = get_plugin_root()
  local python_script = plugin_root .. "util/html_to_md_cli.py"
  local python_executable = plugin_root .. "/.venv/bin/python"

  local input_data = vim.fn.json_encode({ html = html_content })
  ...
  return { title = result.title, content = result.content }, nil
end
